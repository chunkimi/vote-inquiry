...
export const filterWinner = (years, originData) => {
  let isSelected = [
    ...originData.filter((item) => item.is_elected === true)
  ];
...
export function groupCandidates(data, condition ,conditionValue ) {
  let main = data.find((item) => item[condition] === conditionValue && item.role === 0);
  let vice = data.find((item) => item[condition] === conditionValue && item.role === 1);
